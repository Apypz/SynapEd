                        ...
                        <div class="grid gap-6 items-start">
                            <div>
                                ...
                                <div class="flex flex-wrap gap-3 mt-6">
                                    @if(auth()->check() && !empty($course['cta']['is_buy']))
                                        <form action="{{ route('courses.enroll', $course['id']) }}" method="POST">
                                            @csrf
                                            <button type="submit" class="btn-gradient inline-flex items-center justify-center px-6 py-3 rounded-2xl text-white font-semibold">
                                                {{ $course['cta']['label'] }}
                                            </button>
                                        </form>
                                    @else
                    ...
                    <div class="rounded-3xl bg-white/75 border border-white/15 overflow-hidden shadow-lg">
                        ...
                        <div class="p-5 space-y-5">
                            ...
                            @if(auth()->check() && !empty($course['cta']['is_buy']))
                                <form action="{{ route('courses.enroll', $course['id']) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="btn-gradient block w-full px-5 py-3.5 rounded-2xl text-white font-semibold text-center">
                                        {{ $course['cta']['label'] }}
                                    </button>
                                </form>
                            @else
