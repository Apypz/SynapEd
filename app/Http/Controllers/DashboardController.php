...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        // ── ADMIN DATA (REAL FROM DATABASE) ──────────────────────────────────
        $payments = Payment::with(['user', 'course'])->latest()->get();
        $totalRevenue = Payment::where('status', 'Berhasil')->sum('amount');
        ...
        if ($educatorCourses->isEmpty()) {
            $educatorCourses = $courses; // fallback if no specific created yet
        }
        ...
        if ($user->isAdmin()) {
            $stats = [
                ...
                ['label' => 'Pendapatan Real',    'value' => $formattedRevenue, 'icon' => 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z'],
                ['label' => 'Status Sistem',      'value' => 'Optimal',        'icon' => 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z'],
            ];
            ...
            $stats = [
                ...
                ['label' => 'Rating Konten',     'value' => '4.9 ★',                      'icon' => 'M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z'],
            ];
        } else {
            $stats = [
        ...
        return view('dashboard', compact('courses', 'levelMap', 'iconPaths', 'user', 'enrolled', 'stats', 'allUsers', 'payments', 'studentProgress', 'educatorCourses'));
    }
}
